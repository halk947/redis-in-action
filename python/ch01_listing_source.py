...
def get_group_articles(conn, group, page, order='score:'):
    ...
    if not conn.exists(key):  # B
        # 计算给定的一个或多个有序集的交集，
        conn.zinterstore(key,  # C 将结果保存到这个key中
                         ['group:' + group, order],  # C 要计算的set的key值
                         aggregate='max',  # C aggregate 指定了聚合之后交集元素应该使用什么样的 score
                         )
        conn.expire(key, 60)  # D 进行临时存储
...
class TestCh01(unittest.TestCase):
    def setUp(self):
        ...
        self.conn = redis.Redis(
            host='10.0.0.100', port=6379, password='phantom', db=15)
